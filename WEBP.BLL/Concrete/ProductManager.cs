...
namespace WEBP.BLL.Concrete
{
    public class ProductManager : IProductService
    {
        ...
        public async Task<List<UiProduct>> GetAllWithSizeAsync(int page, int pageSize)
        {
            ...
            var products = await _productDal.GetListAsync(pageSize, page, p => p.AktifMi);
        ...
        {
            var products = await _productDal.GetListAsync(p => p.AktifMi);
        ...
        {
            var product = await _productDal.GetAsync(p => p.UniqueId == productId && p.AktifMi );
        ...
        public async Task<List<UiProduct>> GetByQueryAsync(string q)
        {
            ...
            var products = await _productDal.GetListAsync(p => p.Title.ToUpper().Contains(cleanQuery) && p.AktifMi);
        ...
        public async Task<bool> DeleteAsync(Guid productId) => await _productDal.UpdateAsync(new Product { UniqueId = productId, AktifMi = false});
